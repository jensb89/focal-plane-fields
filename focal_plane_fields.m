...
for xx=1:length(x)
    for yy = 1:length(y)
        % rho
        rho = sqrt(x(xx).^2 + y(yy).^2);
        % phi
        if x(xx) > 0
            phi = atan(y(yy)/x(xx));
        elseif x(xx) < 0 && y(yy)>=0
            phi = atan(y(yy)/x(xx)) + pi;
        elseif x(xx) < 0 && y(yy)<0
            phi = atan(y(yy)/x(xx)) + pi;
        elseif x(xx)==0 && y(yy)>0
            phi = pi/2;
        ...
        % fields
        Ex(yy,xx) = 1i*k*f/2 * sqrt(n1/n2) * E_0 * exp(1i*k*f) * (I00 + I02*cos(2*phi));
        Ey(yy,xx) = 1i*k*f/2 * sqrt(n1/n2) * E_0 * exp(1i*k*f) * (I02*sin(2*phi));
        Ez(yy,xx) = 1i*k*f/2 * sqrt(n1/n2) * E_0 * exp(1i*k*f) * (-2*1i*I01*cos(phi));
...
%export_fig(['Ex_Ey_Ez_NA_',num2str(NA),'_z',num2str(z),'.png'],'-dpng','-r200','-transparent')